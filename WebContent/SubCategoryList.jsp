...
<!-- Title -->
<title>Display All Sub Category</title>
...
<script type="text/javascript">
	...
	function getid(catid){
		...
		var cid=catid;
		var strLink="DeleteSubCategoryServlet?id="+cid;
...
<body>
	...
	<jsp:include page="/DisplaySubCategoryDetails"/>
		
	<%
		List<Category> categoeyList = (List) request.getAttribute("SubCategory");
		List<Category> category1=(List)request.getAttribute("Category");
	%>
	...
	<div class="breadcrumb-area">
		...
		<a href="InserSubCategory.jsp" id="#btn"><button style="width:200px; " class="btn alazea-btn"><i class="fa fa-plus-circle" aria-hidden="true">   Add Sub Categoy</i></button></a>
		</div>
	...
	<div style="padding: 50px;">
		...
		<table id="example" class="table table-striped table-bordered nowrap" style="width: 100%; padding: 10px; margin-right: 200px;">
			...
			<tbody>
				...
				<tr>
					<td><%=cnt %></td>
				<% 	for(Category cat1 : category1) { if(cat1.getCat_id()==cat.getCat_id()){%>
					<td><%=cat1.getCat_name() %></td>
					<%} %>
					
				<%} %>
					<td><%=cat.getCat_name() %></td>
				<%-- 	<td><%=cat.getStatus() %></td>   --%>
					<td><a type="button" href="EditSubCategoryDetails?id=<%= cat.getSubcat_id() %>"><i class="fa fa-edit" aria-hidden="true" style="color:green; cursor: pointer; font-size: 25px;"></i></a></td>
			...
			<%if(cat.getStatus()==1 ){ %>
			<td>	<a type="button" data-toggle="modal" data-target="#exampleModalCenter" onclick="getid(<%=cat.getSubcat_id() %>);"><button class="btn btn-success" style="width: 100px; height: 30px; background-color:#70c745;">InActive</button></a></td>
		<%}else{ %>
		<%int i=0; %>
				<%for(Category cat2 : category1){ %>
				<%if(cat2.getStatus()!=0 && cat2.getCat_id()==cat.getCat_id() && i==0)  { %>
				<%System.out.println("category id :"+cat2.getCat_id());%>
				<%System.out.println("Sub category id :"+cat.getCat_id());%>
				<%System.out.println("category status :"+cat2.getStatus());%> 
					<td>	<a type="button" data-toggle="modal" data-target="#exampleModalCenter" onclick="getid(<%=cat.getSubcat_id() %>);"><button class="btn btn-success" style="width: 100px; height: 30px; background-color:#ff0000;">Active</button></a></td>
							<%i++; %>		
			<%}else if(cat2.getCat_id()==cat.getCat_id()){ if(i==0){%>
				<%System.out.println("else category id :"+cat2.getCat_id());%>
				<%System.out.println("else Sub category id :"+cat.getCat_id());%>
				<%System.out.println("else category status :"+cat2.getStatus());%>
								<td>	<a type="button" data-toggle="modal" data-target="#exampleModalCenter" onclick="getid(<%=cat.getSubcat_id() %>);"><button disabled="disabled" class="btn btn-success" style="width: 100px; height: 30px; background-color:#ff0000;">Active</button></a></td>
							<%i++; %>
			<%}}}%>
		<%} %>
		<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">Your All Product Status will be  Change?</div>

...
<body>
<%Category cat=(Category)request.getAttribute("subcategorydetail");
  List<Category> cat1=(List)request.getAttribute("CategoryList");%>
	...
	<div class="container">
		...
		<div class="back"
			...
			<form class="form validity" action="UpdateSubCategoryServlet" method="post">
				
				<h1 style="font-weight: bold; font-size: 30px; text-align: center; margin-bottom: 30px;">Update sub Category</h1>
				...
				<select required class="form-control" name="categoryid" style="width:500px;">
					...
					<% for (Category cat2 : cat1) {%>
					
							<%if(cat.getCat_id()==cat2.getCat_id()){ %>
							<option value="<%=cat2.getCat_id() %>"><%=cat2.getCat_name() %></option><%} %>
				...
				<input type="hidden" name="subcategoryid" value="<%=cat.getSubcat_id() %>">
				<div class="form-group">
					...
					<div class="col-12 mb-4">
						...
						<input id="name2" name="subcategoryname" class="form-control input-group-lg reg_name" data-missing="This field is required" type="text" value="<%=cat.getCat_name() %>" required style="width: 500px;">
				...
				<div class="checkout-btn mt-30">
					<button class="btn alazea-btn" style="margin-left: 300px;">Update Category</button>
